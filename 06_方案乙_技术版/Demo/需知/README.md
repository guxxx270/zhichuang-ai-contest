...
| **估量** | 六维复杂度 → 基础人天；**类比估算**（历史需求库实际人天按相似度加权校准）→ 区间 + 置信度 + 阶段拆分 | 纯程序计算 |
...
xuzhi/pipeline/         intake 问清 · spec 写单 · estimate 估量 · architect 定架 · prototype 出样
...
prompts/                模型润色提示词         tests/  pytest（9 项）
```
...
3. `data/history_requirements.json`：导入工单系统的历史需求与实际工时——估算立刻变准。
4. `xuzhi/textutil.py`：品种、部门、渠道、数据源词典。
