...
不配置 key 也能跑（**mock 模式**，规则引擎兜底）。开工默认只跑规则（毫秒级）。**口述需求**：用 Edge / Chrome 打开页面，点「开始口述」对着麦克风说，说完点「结束口述」，识别结果接到原话框里**当前**内容后面（框里删掉的字不会再拼回来）。Chrome 可能会访问 Google 语音服务。要用大模型润色：打开「开工时自动润色」或开工后在「问清」点「模型润色」，在页面填写 **API Key**（仅当前会话有效）。Qoder 请选 **Cloud Agents** 并填 PAT；也可用硅基流动或自定义 OpenAI 兼容网关。
